...
// showNotification, above) back to the app so it can do the actual
// routing (e.g. camp_pin -> that festival's Map), the same place the
// in-app bell dropdown's tap does. Previously this was a straight
// focus-or-open with the meta thrown away entirely, so tapping a push
// always landed wherever the app happened to already be instead of
// anywhere relevant to the notification.
//
// Two different delivery methods, deliberately -- an already-running
// client (app open/backgrounded) gets a postMessage, which App.jsx is
// already listening for and can act on immediately with no reload. A
// cold start (app fully closed, so openWindow has to launch a fresh
// page) can't rely on that: there's no listener mounted yet to receive
// a message, and no reliable way to know when there will be, so the
// message can just get lost in the race. Encoding the same info as a
// URL query param instead means the fresh load picks it up itself on
// mount (same pattern as the existing ?join= crew-invite link), with
// nothing timing-dependent about it.
